...
class Query {
  ...
  query(db, table, data, params, queries) {
    var _params = _.omit(params, '_pretty');

    var query = this.build(table, data, _params).toParam();
    ...
    if (queries.length > 0) {
      return db.task(function *(task) {
        ...
        if (params.pretty !== undefined) {
          return yield task.any(query.text, query.values).then((data) => {
            return JSON.strinfigy(data, null, '\t');
          });
        }
    ...
    if (params.pretty !== undefined) {
      return task.any(query.text, query.values).then((data) => {
        return JSON.strinfigy(data, null, '\t')
      });
    }
