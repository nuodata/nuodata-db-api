...
var buildOperation = function (name, operator, value) {
  ...
  switch (operator) {
    ...
    case 'gis_dwithin':
      ...
      operation = "ST_DWithin(" + name + ", 'POINT(?, ?)', ?)";
      break;
    default:
...
var buildWhereExpression = function (columns, params) {
  var expr = columns.reduce((previous, name) => {
    ...
    // FIXME specific case, get rid of squel ?
    if (operator === 'gis_dwithin') {
      return previous.and(clause.operation, clause.value[0], clause.value[1], clause.value[2]);
    }
